function alphas = strong_wolfe_line_search(f, g, d, x0)
...
% Modified by H.Kasai on November 1, 2016
    ...
    while true
        ...
        if (fxx > fx0 + c1*alphax*gx0) || ((i > 1) && (fxx >= fxp))
            [alphas,fs,gs] = Zoom(f,g,x0,d,alphap,alphax,fx0,gx0);
            return;
        end
        if abs(gxx) <= -c2*gx0,
            alphas = alphax;
        ...
        if gxx >= 0,
            [alphas,fs,gs] = Zoom(f,g,x0,d,alphax,alphap,fx0,gx0);
            return;
        end
...
%%%%%%%
function [alphas,fs,gs] = Zoom(f,g,x0,d,alphal,alphah,fx0,gx0)
