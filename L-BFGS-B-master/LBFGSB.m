function [x,xhist] = LBFGSB(func, grad, x0,l,u,options, x_star)
...
xhist = [];
if (xhistory)
  xhist = [xhist norm(x0-x_star)/norm(x_star)];
end
...
while ( (get_optimality(x,g,l,u) > tol) && (k < max_iters) )
  ...
  alpha = 1.0;
  if (line_search_flag)
    [alpha] = strong_wolfe(func, grad, x,f,g,xbar-x);
  end
  x = x + alpha * (xbar - x);
  ...
  k = k+1;
  if (xhistory)
    xhist = [xhist norm(x-x_star)/norm(x_star)];
  end
